...
const ArtistShowcase = ({ users, activeId, direction, handleMouseEnter, setActiveId }) => {
  ...
  <h2 className="text-white text-2xl sm:text-3xl md:text-4xl lg:text-5xl font-black uppercase tracking-[0.3em] text-center whitespace-nowrap">
    MOST VISITED ARTISTS
  </h2>
</div>

      {/* Subtle Gradient for Top-Left Text Visibility */}
      <div className="absolute inset-0 bg-linear-to-br from-black/60 via-transparent to-transparent z-10 pointer-events-none"></div>
      ...
      <div className="absolute left-4 md:left-12 top-24 md:top-40 z-30 pointer-events-none max-w-[90vw] md:max-w-none">
        {activeProfile && (
          ...
          >
            ...
            <div className="w-16 md:w-28 h-1 md:h-1.5 bg-linear-to-r from-yellow-400 to-yellow-500/60 mb-3 md:mb-7 shadow-[0_0_20px_rgba(250,204,21,0.7)] rounded-full"></div>
            ...
            <div className="flex flex-col gap-2 md:gap-2">
              ...
              <div className="block items-center gap-2 md:gap-5 flex-wrap">
                <p className="text-white text-xs md:text-xl lg:text-xl font-semibold tracking-widest md:tracking-[0.25em] uppercase drop-shadow-[0_4px_12px_rgba(0,0,0,0.8)]">
                  {activeProfile.role}
                </p>
                <span className="hidden md:block h-8 w-0.5 bg-linear-to-b from-white/60 to-white/20"></span>
      ...
      <div className="container mx-auto px-4 md:px-6 flex flex-col items-center justify-center z-10 w-full max-w-6xl relative pt-8 pb-4">
        <div className="relative flex flex-col items-center justify-center gap-0">
          {/* Main Card - Responsive sizing */}

          <div className="relative w-55 h-75 sm:w-65 sm:h-85 md:w-75 md:h-100">
            {activeId && (
              <div
